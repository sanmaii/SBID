...
def sakura_ui(lang: str):
    ...
    sakura_window.geometry(f'{w_size}x{h_size}')
    sakura_window.minsize(800, 500)
    ...
    def setup_ui(win: CTkToplevel):
        ...
        # Latest blog
        latest_blog_data = sakura.get_latest_blog()
        ...
        latest_blog_label.bind('<Button-1>', lambda x: webbrowser.open(f'https://sakurazaka46.com/s/s46/diary/detail/{latest_blog_num}'))
        ...
        select_dir_btn = CTkButton(dir_btn_frame, text=langconfig['DL']['select'], command=lambda: sakura.select_dir(dir_label), cursor='hand2', width=60)
        select_dir_btn.grid(row=0, column=0, padx=(0, 10))
        open_dir_btn = CTkButton(dir_btn_frame, text=langconfig['DL']['open'], command=lambda: sakura.open_dir(dir_label.cget('text')), cursor='hand2', width=60)
