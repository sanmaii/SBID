...
def hinata_ui(lang: str):
    ...
    hinata_window.geometry('%dx%d' % (w_size, h_size))
    hinata_window.resizable(0,0)
    ...
    def setup_ui(win: CTkToplevel):
        ...
        latest_blog_label = CTkLabel(frame, text=hinata.get_latest_blog()[1], font=CTkFont(size=20), fg_color=('gray60', 'gray40'),
                                     width=w_size-120, cursor='hand2', corner_radius=10)
        latest_blog_label.grid(row=0, column=1, padx=10, pady=10)
        latest_blog_label.bind('<Button-1>', lambda x: webbrowser.open(f'https://www.hinatazaka46.com/s/official/diary/detail/{hinata.get_latest_blog()[0]}'))
        ...
        select_dir_btn = CTkButton(frame, text=langconfig['DL']['select'], command=lambda: hinata.select_dir(dir_label), cursor='hand2', width=60)
        select_dir_btn.grid(row=5, column=1, padx=10, pady=10, sticky='w')
        open_dir_btn = CTkButton(frame, text=langconfig['DL']['open'], command=lambda: hinata.open_dir(dir_label.cget('text')), cursor='hand2', width=60)
