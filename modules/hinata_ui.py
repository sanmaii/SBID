...
def hinata_ui(lang: str):
    # Basic properties
    hinata_window = CTkToplevel()
    hinata_window.title("Hinatazaka46")
    hinata_window.focus()
    w_mon = hinata_window.winfo_screenwidth()
    h_mon = hinata_window.winfo_screenheight()
    w_size = int(w_mon/2.5)
    h_size = int(h_mon/2.2)
    hinata_window.geometry(f'{w_size}x{h_size}')
    hinata_window.minsize(800, 500)
    hinata_window.configure(padx=20, pady=20)
    ...
    def setup_ui(win: CTkToplevel):
        ...
        # Latest blog
        latest_blog_data = hinata.get_latest_blog()
        latest_blog_num = latest_blog_data[0]
        latest_blog_title = latest_blog_data[1]
        latest_blog_text = CTkLabel(frame, text=langconfig['DL']['latest_blog'])
        latest_blog_text.grid(row=0, column=0, padx=10, pady=10)
        latest_blog_label = CTkLabel(frame, text=latest_blog_title, font=CTkFont(size=20), fg_color=('gray60', 'gray40'),
                                     width=w_size-120, cursor='hand2', corner_radius=10)
        latest_blog_label.grid(row=0, column=1, padx=10, pady=10)
        latest_blog_label.bind('<Button-1>', lambda x: webbrowser.open(f'https://www.hinatazaka46.com/s/official/diary/detail/{latest_blog_num}'))
        ...
        member_menu.configure(values=hinata.get_members(), command=lambda x: hinata.get_blogs(member_menu.get(), blog_menu, lang, langconfig, dir_label, image_num_entry))
        ...
        dir_label.grid(row=4, column=1, padx=10, pady=10, sticky='ew')

        # Directory related buttons
        dir_btn_frame = CTkFrame(frame, fg_color='transparent')
        dir_btn_frame.grid(row=5, column=1, padx=10, pady=10, sticky='w')
        select_dir_btn = CTkButton(dir_btn_frame, text=langconfig['DL']['select'], command=lambda: hinata.select_dir(dir_label), cursor='hand2', width=60)
        select_dir_btn.grid(row=0, column=0, padx=(0, 10))
        open_dir_btn = CTkButton(dir_btn_frame, text=langconfig['DL']['open'], command=lambda: hinata.open_dir(dir_label.cget('text')), cursor='hand2', width=60)
        open_dir_btn.grid(row=0, column=1)
        ...
        dl_btn_frame = CTkFrame(frame, fg_color='transparent')
        dl_btn_frame.grid(row=7, column=1, padx=10, pady=10, sticky='w')
        dl_single_btn = CTkButton(dl_btn_frame, width=120, fg_color='#910c14', text=langconfig['DL']['dl_single'], hover_color='#6e0b12',
                                command=lambda: hinata.download(member_menu.get(), lang, member_menu, blog_menu, blog_entry, select_dir_btn,
                                open_dir_btn,image_num_entry, dl_single_btn, dl_latest_btn, dir_label, 'single'))
        dl_single_btn.grid(row=0, column=0, padx=(0, 10))       
        dl_latest_btn = CTkButton(dl_btn_frame, width=120, fg_color='#910c14', text=langconfig['DL']['dl_latest'], hover_color='#6e0b12',
                                  command=lambda: hinata.download(member_menu.get(), lang, member_menu, blog_menu, blog_entry, select_dir_btn,
                                open_dir_btn,image_num_entry, dl_single_btn, dl_latest_btn, dir_label, 'latest'))
        dl_latest_btn.grid(row=0, column=1)   

    setup_ui(hinata_window)