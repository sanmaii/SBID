import n46_get_latest_blog_info
...
# Get the latest blog
def get_latest_blog():
    latest_blog_num, title, member, date = n46_get_latest_blog_info.get_n46_latest_blog_info()
    return latest_blog_num, f'{latest_blog_num} --- {title.strip()}\n{member.strip()} {date.replace(".", "/")} (GMT+9)'

# Get the members from the member list
def get_members():
    return mem.fetch_member_list()['n46members']
...
def get_blogs(member: str, blog_menu: CTkOptionMenu, lang: str, langconfig: configparser.ConfigParser, dir_label: CTkLabel, img_num_entry: CTkEntry):
    mem_config_file = 'members/n46mem.ini'
    mem_config = configparser.ConfigParser()
    mem_config.read(mem_config_file, encoding='utf-8')
    domain = 'https://www.nogizaka46.com/s/n46/diary/MEMBER/list?ct='
    mid = mem.fetch_member_list()['n46dict'].get(member)
    ...
    while count < blog_page:
        try:
            ...
            blog_count = len(bloglist_source.select('div.bl--list a'))
            half = blog_count/2
            if half < 1:
                half == 1
            else:
    ...
    if last_blog == '':
        ...
        for i in range(2):
            ...
            if i == 1:
                blog_title = source.select('h1.bd--hd__ttl.f--head.a--tx.js-tdi')[0].text
...
def download(member: str, lang: str, mem_menu: CTkOptionMenu, blog_menu: CTkOptionMenu, blog_entry: CTkEntry,
    ...
    output_ui.output_ui()
    bid, directory, img_num, dl_count = n46dl.dl(img_num, url, dir_label.cget('text'), mode)
