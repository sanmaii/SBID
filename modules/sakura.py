import s46_get_latest_blog_info
...
# Get the latest blog
def get_latest_blog():
    latest_blog_num, title, member, date = s46_get_latest_blog_info.get_s46_latest_blog_info()
    return latest_blog_num, f'{latest_blog_num} --- {title.strip()}\n{member.strip()} {date.replace(".", "/")} (GMT+9)'

# Get the members from the member list
def get_members():
    return mem.fetch_member_list()['s46members']
...
def get_blogs(member: str, blog_menu: CTkOptionMenu, lang: str, langconfig: configparser.ConfigParser, dir_label: CTkLabel, img_num_entry: CTkEntry):
    mem_config_file = 'members/s46mem.ini'
    mem_config = configparser.ConfigParser()
    mem_config.read(mem_config_file, encoding='utf-8')
    domain = 'https://sakurazaka46.com/s/s46/diary/blog/list?ct='
    mid = mem.fetch_member_list()['s46dict'].get(member)
    ...
    while count < blog_page:
        try:
            ...
            blog_count = len(bloglist_source.select('div.member-blog-listm')[0].select('li.box'))
            half = blog_count/2
            if half < 1:
                half == 1
            else:
            ...
            def get_half(small_num: int, big_num: int):
                while small_num < big_num:                   
                    blog_order = bloglist_source.select('li.box a')[small_num]['href']
                    blog_num = blog_order[blog_order.rfind('/')+1:blog_order.rfind('?')]
                    blog_title = bloglist_source.select('li.box h3.title')[small_num].text
...
def download(member: str, lang: str, mem_menu: CTkOptionMenu, blog_menu: CTkOptionMenu, blog_entry: CTkEntry,
    ...
    if blog_menu.get() != '-' and not blog_menu.get().startswith('*'):
        ...
        if len(blog_entry.get()) == 5:
            if req.head(main + temp_bid).status_code != 200:
                ...
                ts_name = temp_source.select('p.name')[0].text
    ...
    output_ui.output_ui()
    bid, directory, img_num, dl_count = s46dl.dl(img_num, url, dir_label.cget('text'), mode)
