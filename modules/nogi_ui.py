...
def nogi_ui(lang: str):
    # Basic properties
    nogi_window = CTkToplevel()
    nogi_window.title("Nogizaka46")
    nogi_window.focus()
    w_mon = nogi_window.winfo_screenwidth()
    h_mon = nogi_window.winfo_screenheight()
    w_size = int(w_mon/2.5)
    h_size = int(h_mon/2.2)
    nogi_window.geometry(f'{w_size}x{h_size}')
    nogi_window.minsize(800, 500)
    nogi_window.configure(padx=20, pady=20)
    ...
    def setup_ui(win: CTkToplevel):
        ...
        # Latest blog
        latest_blog_data = nogi.get_latest_blog()
        ...
        latest_blog_label.bind('<Button-1>', lambda x: webbrowser.open(f'https://www.nogizaka46.com/s/n46/diary/detail/{latest_blog_num}'))
        ...
        member_menu.configure(values=nogi.get_members(), command=lambda x: nogi.get_blogs(member_menu.get(), blog_menu, lang, langconfig, dir_label, image_num_entry))
        ...
        select_dir_btn = CTkButton(dir_btn_frame, text=langconfig['DL']['select'], command=lambda: nogi.select_dir(dir_label), cursor='hand2', width=60)
        select_dir_btn.grid(row=0, column=0, padx=(0, 10))
        open_dir_btn = CTkButton(dir_btn_frame, text=langconfig['DL']['open'], command=lambda: nogi.open_dir(dir_label.cget('text')), cursor='hand2', width=60)
        ...
        dl_single_btn = CTkButton(dl_btn_frame, width=120, fg_color='#910c14', text=langconfig['DL']['dl_single'], hover_color='#6e0b12',
                                command=lambda: nogi.download(member_menu.get(), lang, member_menu, blog_menu, blog_entry, select_dir_btn,
